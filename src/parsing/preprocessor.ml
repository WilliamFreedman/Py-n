...
let should_ignore str =
  if String.for_all (fun x -> x = ' ') str then
    Printf.printf "space condition\n"
  else if String.length str = 0 then Printf.printf "zero condition\n";
  (String.for_all (fun x -> x = ' ')) str || String.length str = 0

(*why is this function returning true on line 1?*)
  ...
  let indent_change =
    if indent_diff > current_indent then
      if (indent_diff - current_indent) mod 4 != 0 then
        raise (Failure ("Indentation error in line " ^ line))
